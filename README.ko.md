...
- **Puck** — 펫 본체: 항상 위에 떠 있는 캐릭터가 화면을 걸어 다니고, 뭔가를 가리키고,
  음성을 듣고, 다른 앱의 화면을 읽고 입력하고, Mac을 조작합니다 (`run_shell`,
  `run_applescript`, UI 요소 클릭/찾기, 앱 실행).
- **PuckClient** — 펫의 창: 채팅, 워크스페이스, git 상태, 네이티브 SwiftUI 코드
  에디터, 터미널 패널.

대화는 껐다 켜도 남습니다. 에이전트는 호출이 끝나도 살아 있는 셸(개발 서버,
워처 같은 것)을 띄워 둘 수 있고, 정해진 때에 실행할 일을 맡길 수도 있습니다
("매일 아침 CI 확인해줘" — Puck이 켜져 있는 동안 실행됩니다). 바꾼 것을
받아들이기 전에는 ⇧⌘R로 파일별 diff를 보고, 마음에 안 드는 파일은 되돌릴 수
있습니다.
...
스스로 업데이트하지는 않습니다. 하루에 한 번 GitHub에 새 릴리스가 올라왔는지 물어
보고, 있으면 펫이 알려줍니다. 버전과 링크는 설정 → 일반에 남아 있고, 받아서 넣는
것은 여전히 사용자 몫입니다. 이 확인은 같은 자리에서 끌 수 있습니다.
