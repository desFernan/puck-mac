...
![아일랜드 바닥에 앉아 있는 펫](.github/media/island.png)
...
설정에 이 폴더를 여는 버튼이 있고 (아바타 → 커스터마이징 폴더 열기), 아직
폴더가 없으면 만들어 주기도 합니다.
...
3. **PNG 한 장과 `manifest.json` 넣기.** 그림 한 장이면 작동하는 캐릭터가
   ...
   `hitbox`는 그려지고 클릭되는 크기(포인트 단위)입니다 — 그림의 비율에
   맞추지 않으면 찌그러져 보입니다.
...
- **`hitbox`**는 `scale` 1일 때 캐릭터의 크기(포인트)입니다 — 펫이 클릭되고,
  서고, 던져지는 기준이 됩니다. **`bounce_intensity`** (0–1)는 정지된
  그림에서 squash-and-stretch가 얼마나 보이는지입니다.
